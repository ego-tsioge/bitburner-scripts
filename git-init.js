// https://github.com/alainbryden/bitburner-scripts/blob/main/git-pull.js
...
const argsSchema = [
	...
	['branch', 'main'],
	  // download: By default, all supported files in the repository will be downloaded. 
	  // Override with just a subset of files here
	['download', []], // 
	  // new-file: If a repository listing fails, only files returned by ns.ls() will be downloaded.
	  // You can add additional files to seek out here.
	['new-file', []], // 
...
export async function main(ns) {
    ...
    const baseUrl = `raw.githubusercontent.com/${options.githubaccount}/${options.repository}/${options.branch}/`;
...
async function repositoryListing(ns, folder = '') {
    ...
    const listUrl = `https://api.github.com/repos/${options.githubaccount}/${options.repository}/contents/${folder}?ref=${options.branch}`
    let response = null;
    try {
...
/*
const baseUrl = 'https://raw.githubusercontent.com/ego-tsioge/bitburner-scripts/main/src/'
...
export async function main(ns) {
	ns.tprint(`[${localeHHMMSS()}] Starting git-init.js`)
	...
	for (let i = 0; i < filesToDownload.length; i++) {
		...
		try {
			...
			await ns.wget(path, "/scr/" + filename);

		}
		catch {
			ns.tprint(`[${localeHHMMSS()}] wget auf #i:${i} mit path: "${path}" schlägt fehl :(`);
			break;
		}
	}
